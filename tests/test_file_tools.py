...
def tearDown():
    ...
    cm = file_tools.ContentManager()
    test_dir = os.path.join(cm.arxain_path, 'authors', 'test_author')
    shutil.rmtree(test_dir)

def test_init():
    """create ContentManager object"""
...
def test_init_author():
    ...
    cm = file_tools.ContentManager()
    result = cm.init_author('test_author')
    print(result)
    assert(result['Success'] == True)

    # try again to re-init
    result2 = cm.init_author('test_author')

    assert(result['peerID'] == result2['peerID'])

    #try again
    results = cm.init_author('test_author')
    assert(results['Success'] == True)
...
def test_submit_manuscript():
    ...
    result = cm.submit_manuscript('test_author', 'test_paper', os.path.join(curr_dir, 'test-paper'))
    print(result)
    assert(result['Success'] == True)

    # submit with wrong author
    result = cm.submit_manuscript('not_an_initiallized_author', 'test_paper', os.path.join(curr_dir, 'test-paper'))
    print(result)
    assert(result['Success'] == False)

    # submit an empty directory
    result = cm.submit_manuscript('test_author', 'test_paper', os.path.join(curr_dir, 'test-empty'))
    assert(result['Success'] == False)
...
def test_submit_revision():
    ...
    # Get the test pdf in the folder
    curr_dir = os.path.dirname(os.path.abspath(__file__))
    result = cm.submit_revision('test_author', 'test_paper', os.path.join(curr_dir, 'test-paper-rev'))
    print(result)
    assert(result['Success'] == True)

    # submit with wrong author
    result = cm.submit_revision('not_an_initiallized_author', 'test_paper', os.path.join(curr_dir, 'test-paper-rev'))

    assert(result['Success'] == False)

    # submit a non intialized paper
    result = cm.submit_revision('test_author', 'NotAPaper', os.path.join(curr_dir, 'test-paper-rev'))

    assert(result['Success'] == False)

    # submit an empty directory
    result = cm.submit_revision('test_author', 'test_paper', os.path.join(curr_dir, 'test-empty'))
    assert(result['Success'] == False)

    # submit a redundant paper
    result = cm.submit_revision('test_author', 'test_paper', os.path.join(curr_dir, 'test-paper'))
    print(result)
    assert(result['Success'] == False)
...
def test_submit_comment():
    ...
    # Get the test pdf in the folder
    curr_dir = os.path.dirname(os.path.abspath(__file__))
    result = cm.submit_comment('test_author', 'test_paper', os.path.join(curr_dir, 'test-comment'))
    print(result)
    assert(result['Success'] == True)

    # submit with wrong author
    result = cm.submit_comment('not_an_initiallized_author', 'test_paper', os.path.join(curr_dir, 'test-comment'))

    assert(result['Success'] == False)

    # submit an empty directory_check# submit an empty directory
    result = cm.submit_comment('test_author', 'test_paper', os.path.join(curr_dir, 'test-empty'))
    assert(result['Success'] == False)
...
def test_check_author_valid():
    ...
    result_valid = cm.check_author('test_author')

    assert(result_valid['Success'] == True)

    # Check an invalid author
    result_false = cm.check_author('NotAnAuthor!')
...
def test_get_extension():
    ...
    curr_dir = os.path.dirname(os.path.abspath(__file__))
    file_names = cm.get_extension('pdf', os.path.join(curr_dir, 'test-paper'))
...
def test_hash_files():
    ...
    hash_list = cm.hash_files(conf_free_files)
...
def test_verify_unique_revision():
    ...
    result = cm.verify_unique_revision(prev_hashes, current_hash)
    ...
    hash_list = cm.hash_files(conf_filled_files)
